...
public class MoreThanTwoHoursGroundTimeFilterImplTest {
    ...
    @Test
    public void check() {
        testFlightsForFiltering.stream().filter(x -> new MoreThanTwoHoursGroundTimeFilterImpl().check(x)).forEach(System.out::println);
        assertArrayEquals(testFlights.toArray(), testFlightsForFiltering.stream().filter(x -> new MoreThanTwoHoursGroundTimeFilterImpl().check(x)).toArray());
        testFlightsForFiltering.stream().filter(x -> new MoreThanTwoHoursGroundTimeFilterImpl().check(x)).forEach(System.out::println);
    }
    ...
    private static List<Flight> buildTestFlightsForFiltering() {
        return Arrays.asList(
                ...
                createFlight(LocalDateTime.of(2021, 6, 23, 2, 53), LocalDateTime.of(2021, 6, 23, 4, 53),
                        LocalDateTime.of(2021, 6, 23, 7, 53), LocalDateTime.of(2021, 6, 23, 6, 53)));
    }
